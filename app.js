...
(function(){
    ...
    scanForm.addEventListener('submit', async (e) => {
      ...
      selectedFiles.forEach((it, idx) => data.append('files', it.file, it.file.name));
      data.append('keywords', $('#keywords').value);
      data.append('links', $('#links').value);
      data.append('watchlist', $('#watchlist').value);
      data.append('scanMode', $('#scan-mode').value);
      ...
      try {
        ...
        if(mode === 'harassment') selectTab('threat');
        else if(mode === 'piracy') selectTab('piracy');
        else selectTab('reports'); // Default to reports for 'both' mode
      } catch(err){
        console.error(err);
        announce('Error: ' + (err.message || 'Scan failed'), 'error');
      } finally {
        hideScanning();
      }
    });
    ...
    function initThreatTabOnce(){ if(threatInit) return; threatInit = true;
      ...
      $('#export-flagged').addEventListener('click', () => {
        const rows = [['Handle','Platform','Risk','LastSeen']].concat(currentFlagged.map(f => [f.handle, f.platform, f.risk, f.lastSeen]));
    ...
    function populateReports(reports, history){
      ...
      history.forEach(h => {
        const li = document.createElement('li');
        li.innerHTML = `<div>${h.date} • Mode: ${h.mode} • Files: ${h.files}</div><span class="badge ${h.status==='Completed'?'success':(h.status==='Error'?'danger':'warn')}">${h.status}</span>`;
        hl.appendChild(li);
      });
    }
